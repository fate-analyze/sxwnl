...
{                                         //行星的距角,jing为精度控
    auto a = p_coord(0, t, 10, 10, 10);   //地球
    auto z = p_coord(xt, t, 10, 10, 10);  //行星
    z = h2g(z, a);                        //转到地心
    //if (jing == 0) ;			//低精度
    if (jing == 1) {                     //中精度
...
{  //行星的视坐标
    double E = hcjj(t);
    const auto a = p_coord(0, t - gxs, n, n, n);  //地球
    auto z = p_coord(xt, t - gxs, n, n, n);       //行星
    z = h2g(z, a);                                //转到地心
    if (gxs) {                                    //如果计算了光行时，那么也计算章动
        std::array<double, 2> zd = nutation2(t);  //章动计算
        z[0] += zd[0];
        E += zd[1];
    }
    z = llrConv(z, E);
...
{  //留,sn=1顺留
    ...
    for (i = 0; i < 4; i++) {
        ...
        std::array<double, 3> y1 = xingLiu0(xt, t - dt, n, g);
        y2 = xingLiu0(xt, t, n, g);
        std::array<double, 3> y3 = xingLiu0(xt, t + dt, n, g);
...
//合月计算
std::array<double, 4> xingMP(int xt, double t, int n, double E, const std::array<double, 4> &g)
{                                                  //月亮行星视赤经差
    const auto a = p_coord(0, t - g[1], n, n, n);  //地球
    auto p = p_coord(xt, t - g[1], n, n, n);       //行星
    auto m = m_coord(t - g[0], n, n, n);           //月亮
    p = h2g(p, a);
    m[0] += g[2];
    p[0] += g[2];
    m = llrConv(m, E + g[3]);
    p = llrConv(p, E + g[3]);
    std::array<double, 4> re = {rad2rrad(m[0] - p[0]), m[1] - p[1], m[2] / cs_GS / 86400 / 36525.0,
...
{  //行星合月(视赤经),t儒略世纪TD
    std::array<double, 4> d, g = {0, 0, 0, 0};
    ...
    d = xingMP(xt, t, 8, E, g);
    std::array<double, 4> d2 = xingMP(xt, t + 1e-6, 8, E, g);
    double v = (d2[0] - d[0]) / 1e-6;  //速度
...
{                                                //行星太阳视黄经差与w0的差
    const auto a = p_coord(0, t - tp, n, n, n);  //地球
    auto p = p_coord(xt, t - tp, n, n, n);       //行星
    auto s = p_coord(0, t - ts, n, n, n);
    s[0] += _pi;
    s[1] = -s[1];  //太阳
    p = h2g(p, a);
...
{  //xt星体号,t儒略世纪TD,f=1求冲(或下合)否则求合(或下合)
    ...
    double v = pi2 / cs_xxHH[xt - 1] * 36525;
    if (xt > 2)
        v = -v;  //行星相对地球的黄经平速度
    for (double i = 0; i < 6; i++)
        t -= rad2rrad(XL0_calc(xt, 0, t, 8) - XL0_calc(0, 0, t, 8) - w0) / v;  //水星的平速度与真速度相差较多,所以多算几次
    //严格计算
    auto a = xingSP(xt, t, 8, w1, 0, 0);
    const auto b = xingSP(xt, t + dt, 8, w1, 0, 0);
    v = (b[0] - a[0]) / dt;
...
{  //行星计算,jd力学时
    ...
    if (xt == 10) {  //月亮
        ...
        s += "视黄经 " + rad2str(z[0], false) + " 视黄纬 " + rad2str(z[1], false) + " 地心距 " + to_str(ra, rfn) + "\n";

        z = llrConv(z, E);  //转到赤道坐标
        s += "视赤经 " + rad2str(z[0], true) + " 视赤纬 " + rad2str(z[1], false) + " 光行距 " + to_str(rb, rfn) + "\n";
    } else if (xt < 10 && xt >= 0) {
        a = p_coord(0, T, -1, -1, -1);   //地球
        z = p_coord(xt, T, -1, -1, -1);  //行星
        z[0] = rad2mrad(z[0]);
        s += "黄经一 " + rad2str(z[0], false) + " 黄纬一 " + rad2str(z[1], false) + " 向径一 " + to_str(z[2], rfn) + "\n";
    ...
    s += "站赤经 " + rad2str(z[0], true) + " 站赤纬 " + rad2str(z[1], false) + " 视距离 " + to_str(rc, rfn) + "\n";
...
{  //求空间两点连线与地球的交点(靠近点x1的交点)
    double dx = x2 - x1, dy = y2 - y1, dz = z2 - z1, e2 = e * e;
    COORDP p = {};
    double A = dx * dx + dy * dy + dz * dz / e2;
    double B = x1 * dx + y1 * dy + z1 * dz / e2;
    double C = x1 * x1 + y1 * y1 + z1 * z1 / e2 - r * r;
    p.D = B * B - A * C;
    if (p.D < 0)
        return p;  //判别式小于0无解
    double D = sqrt(p.D);
...
COORDP lineEar2(double x1, double y1, double z1, double x2, double y2, double z2, double e, double r, const std::array<double, 3> &I)
{  //I是贝塞尔坐标参数
...
COORDP lineEar(const std::array<double, 3> &P, const std::array<double, 3> &Q, double gst)
...
{  //椭圆与圆的交点,R椭圆长半径,R2圆半径,x0,y0圆的圆心
    ...
    for (int k = -1; k < 2; k += 2) {
        ...
        double C = cosA * cosB - sinA * sinB * k;
...
NODE lineOvl(double x1, double y1, double dx, double dy, double r, double ba)
{
    ...
    NODE p = {};
    double f = ba * ba;
    double A = dx * dx + dy * dy / f;
    double B = x1 * dx + y1 * dy / f;
    double C = x1 * x1 + y1 * y1 / f - r * r;
    double D = B * B - A * C;
    if (D < 0) {
        p.n = 0;
        return p;
...
{  //快速日食搜索,jd为朔时间(J2000起算的儒略日数,不必很精确)
    ...
    L = -1.084719 + 7771.377145013 * t - 0.0000331 * t2
        + (22640 * cos(0.785 + 8328.6914 * t + 0.000152 * t2) + 4586 * cos(0.19 + 7214.063 * t - 0.000218 * t2)
           + 2370 * cos(2.54 + 15542.754 * t - 0.000070 * t2) + 769 * cos(3.1 + 16657.383 * t) + 666 * cos(1.5 + 628.302 * t)
           + 412 * cos(4.8 + 16866.93 * t) + 212 * cos(4.1 - 1114.63 * t) + 205 * cos(0.2 + 6585.76 * t) + 192 * cos(4.9 + 23871.45 * t)
           + 165 * cos(2.6 + 14914.45 * t) + 147 * cos(5.5 - 7700.39 * t) + 125 * cos(0.5 + 7771.38 * t) + 109 * cos(3.9 + 8956.99 * t)
           + 55 * cos(5.6 - 1324.18 * t) + 45 * cos(0.9 + 25195.62 * t) + 40 * cos(3.8 - 8538.24 * t) + 38 * cos(4.3 + 22756.82 * t)
           + 36 * cos(5.5 + 24986.07 * t) - 6893 * cos(4.669257 + 628.3076 * t) - 72 * cos(4.6261 + 1256.62 * t)
           - 43 * cos(2.67823 + 628.31 * t) * t + 21)
              / rad;
    t += (W - L)
         / (7771.38 - 914 * sin(0.7848 + 8328.691425 * t + 0.0001523 * t2) - 179 * sin(2.543 + 15542.7543 * t)
            - 160 * sin(0.1874 + 7214.0629 * t));
    re.jd = re.jdSuo = jd = t * 36525;  //朔时刻

    //纬 52,15 (角秒)
    t2 = t * t / 10000, t3 = t2 * t / 10000;
    mB = 18461 * cos(0.0571 + 8433.46616 * t - 0.640 * t2 - 1 * t3) + 1010 * cos(2.413 + 16762.1576 * t + 0.88 * t2 + 25 * t3)
         + 1000 * cos(5.440 - 104.7747 * t + 2.16 * t2 + 26 * t3) + 624 * cos(0.915 + 7109.2881 * t + 0 * t2 + 7 * t3)
         + 199 * cos(1.82 + 15647.529 * t - 2.8 * t2 - 19 * t3) + 167 * cos(4.84 - 1219.403 * t - 1.5 * t2 - 18 * t3)
         + 117 * cos(4.17 + 23976.220 * t - 1.3 * t2 + 6 * t3) + 62 * cos(4.8 + 25090.849 * t + 2 * t2 + 50 * t3)
         + 33 * cos(3.3 + 15437.980 * t + 2 * t2 + 32 * t3) + 32 * cos(1.5 + 8223.917 * t + 4 * t2 + 51 * t3)
         + 30 * cos(1.0 + 6480.986 * t + 0 * t2 + 7 * t3) + 16 * cos(2.5 - 9548.095 * t - 3 * t2 - 43 * t3)
         + 15 * cos(0.2 + 32304.912 * t + 0 * t2 + 31 * t3) + 12 * cos(4.0 + 7737.590 * t) + 9 * cos(1.9 + 15019.227 * t)
         + 8 * cos(5.4 + 8399.709 * t) + 8 * cos(4.2 + 23347.918 * t) + 7 * cos(4.9 - 1847.705 * t) + 7 * cos(3.8 - 16133.856 * t)
         + 7 * cos(2.7 + 14323.351 * t);
    mB /= rad;

    //距 106, 23 (千米)
    mR = 385001 + 20905 * cos(5.4971 + 8328.691425 * t + 1.52 * t2 + 25 * t3) + 3699 * cos(4.900 + 7214.06287 * t - 2.18 * t2 - 19 * t3)
         + 2956 * cos(0.972 + 15542.75429 * t - 0.66 * t2 + 6 * t3) + 570 * cos(1.57 + 16657.3828 * t + 3.0 * t2 + 50 * t3)
         + 246 * cos(5.69 - 1114.6286 * t - 3.7 * t2 - 44 * t3) + 205 * cos(1.02 + 14914.4523 * t - 1 * t2 + 6 * t3)
         + 171 * cos(3.33 + 23871.4457 * t + 1 * t2 + 31 * t3) + 152 * cos(4.94 + 6585.761 * t - 2 * t2 - 19 * t3)
         + 130 * cos(0.74 - 7700.389 * t - 2 * t2 - 25 * t3) + 109 * cos(5.20 + 7771.377 * t)
         + 105 * cos(2.31 + 8956.993 * t + 1 * t2 + 25 * t3) + 80 * cos(5.38 - 8538.241 * t + 2.8 * t2 + 26 * t3)
         + 49 * cos(6.24 + 628.302 * t) + 35 * cos(2.7 + 22756.817 * t - 3 * t2 - 13 * t3) + 31 * cos(4.1 + 16171.056 * t - 1 * t2 + 6 * t3)
         + 24 * cos(1.7 + 7842.365 * t - 2 * t2 - 19 * t3) + 23 * cos(3.9 + 24986.074 * t + 5 * t2 + 75 * t3)
         + 22 * cos(0.4 + 14428.126 * t - 4 * t2 - 38 * t3) + 17 * cos(2.0 + 8399.679 * t);
    mR /= 6378.1366;

    t = jd / 365250, t2 = t * t, t3 = t2 * t;
    //误0.0002AU
    sR = 10001399  //日地距离
         + 167070 * cos(3.098464 + 6283.07585 * t) + 1396 * cos(3.0552 + 12566.1517 * t) + 10302 * cos(1.10749 + 6283.07585 * t) * t
         + 172 * cos(1.064 + 12566.152 * t) * t + 436 * cos(5.785 + 6283.076 * t) * t2 + 14 * cos(4.27 + 6283.08 * t) * t3;
