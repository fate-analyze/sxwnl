...
{  //计算世界时与原子时之差,传入年
    double dt_at[] = {-4000,  108371.7, -13036.80, 392.000, 0.0000,  -500,    17201.0, -627.82, 16.170,  -0.3413, -150,    12200.6, -346.41,
                      5.403,  -0.1593,  150,       9113.8,  -328.13, -1.647,  0.0377,  500,     5707.5,  -391.41, 0.915,   0.3145,  900,
                      2203.4, -283.45,  13.034,    -0.1778, 1300,    490.1,   -57.35,  2.085,   -0.0072, 1600,    120.0,   -9.81,   -1.532,
                      0.1403, 1700,     10.2,      -0.91,   0.510,   -0.0370, 1800,    13.4,    -0.72,   0.202,   -0.0193, 1830,    7.8,
                      -1.81,  0.416,    -0.0247,   1860,    8.3,     -0.13,   -0.406,  0.0292,  1880,    -5.4,    0.32,    -0.183,  0.0173,
                      1900,   -2.3,     2.06,      0.169,   -0.0135, 1920,    21.2,    1.69,    -0.304,  0.0167,  1940,    24.2,    1.22,
                      -0.064, 0.0031,   1960,      33.2,    0.51,    0.231,   -0.0109, 1980,    51.0,    1.29,    -0.026,  0.0032,  2000,
                      63.87,  0.1,      0,         0,       2005,    64.7,    0.21,    0,       0,       2012,    66.8,    0.22,    0,
                      0,      2018,     69.0,      0.36,    0,       0,       2028,    72.6};
...
{  //视差修正
    ...
    double r0 = cs_rEar * cos(u) + high * cos(fa);      //站点与地地心向径的赤道投影长度
    double z0 = cs_rEar * sin(u) * f + high * sin(fa);  //站点与地地心向径的轴向投影长度
    double x0 = r0 * cos(g);
    double y0 = r0 * sin(g);
...
{  //中精度章动计算,t是世纪数
    double a, t2 = t * t, *B = nutB, dL = 0, dE = 0;
    for (int i = 0; i < sizeof(nutB) / sizeof(double); i += 5) {
        double c = B[i] + B[i + 1] * t + B[i + 2] * t2;
        if (i == 0)
            a = -1.742 * t;
        else
            a = 0;
...
{  // 返回冥王星J2000直角坐标
    ...
    int len[] = {
        sizeof(PL0) / sizeof(double), sizeof(PL1) / sizeof(double), sizeof(PL2) / sizeof(double),
        sizeof(PL3) / sizeof(double), sizeof(PL4) / sizeof(double), sizeof(PL5) / sizeof(double),
        sizeof(PL6) / sizeof(double), sizeof(PL7) / sizeof(double), sizeof(PL8) / sizeof(double),
    };
    for (int i = 0; i < 9; i++) {
...
{                                                                                  //月球速度计算,传入世经数
    ...
    v -= 179 * sin(2.543 + 15542.7543 * t)                                         //误差小于0.3%
         + 160 * sin(0.1874 + 7214.0629 * t) + 62 * sin(3.14 + 16657.3828 * t) + 34 * sin(4.827 + 16866.9323 * t)
         + 22 * sin(4.9 + 23871.4457 * t) + 12 * sin(2.59 + 14914.4523 * t) + 7 * sin(0.23 + 6585.7609 * t) + 5 * sin(0.9 + 25195.624 * t)
...
{  //已知月日视黄经差求时间,高速低精度,误差不超过600秒(只验算了几千年)
    ...
    t -= (-0.00003309 * t2 + 0.10976 * cos(0.784758 + 8328.6914246 * t + 0.000152292 * t2)
          + 0.02224 * cos(0.18740 + 7214.0628654 * t - 0.00021848 * t2) - 0.03342 * cos(4.669257 + 628.307585 * t))
         / v;
    double L = M_Lon(t, 20)
...
{  //求月亮近点时间和距离,t为儒略世纪数力学时
    ...
    double r2 = XL1_calc(2, t, 10);
    double r3 = XL1_calc(2, t + dt, 10);
...
{  //地球近远点
    ...
    double r2 = XL0_calc(0, 2, t, 10);
    double r3 = XL0_calc(0, 2, t + dt, 10);
...
{  //月亮被照亮部分的比例
    ...
    double dm = _pi / 180;
    double D = (297.8502042 + 445267.1115168 * t - 0.0016300 * t2 + t3 / 545868 - t4 / 113065000) * dm;  //日月平距角
    double M = (357.5291092 + 35999.0502909 * t - 0.0001536 * t2 + t3 / 24490000) * dm;                  //太阳平近点
    double m = (134.9634114 + 477198.8676313 * t + 0.0089970 * t2 + t3 / 69699 - t4 / 14712000) * dm;    //月亮平近点
    double a =
        _pi - D
        + (-6.289 * sin(m) + 2.100 * sin(M) - 1.274 * sin(D * 2 - m) - 0.658 * sin(D * 2) - 0.214 * sin(m * 2) - 0.110 * sin(D)) * dm;
    return (1 + cos(a)) / 2;
}
