...
std::string MSC::toStr(bool fs)
{
    std::stringstream ss;
    ss << "-------------------------------------------\n";
    ss << "平太阳 " << timeStr(MSC::pty) << " 真太阳 " << timeStr(MSC::zty) << "\n";
    ss << "时差 " << m2fm(MSC::sc * 86400, 2, 1) << " 月亮被照亮 " << to_str(MSC::mIll * 100, 2) << "% ";
    ss << "\n";

    ss << "-------------------------------------------\n表一       月亮            太阳\n";
    ss << "视黄经 " << rad2str(MSC::mHJ, false) << "  " << rad2str(MSC::sHJ, false) << "\n";
    ss << "视黄纬 " << rad2str(MSC::mHW, false) << "  " << rad2str(MSC::sHW, false) << "\n";
    ss << "视赤经 " << rad2str(MSC::mCJ, true) << "  " << rad2str(MSC::sCJ, true) << "\n";
    ss << "视赤纬 " << rad2str(MSC::mCW, false) << "  " << rad2str(MSC::sCW, false) << "\n";
    ss << "距离    " << to_str(MSC::mR, 2) << "千米    " << to_str(MSC::sR, 8) << "AU" << "\n";

    ss << "-------------------------------------------\n表二       月亮            太阳\n";
    ss << "方位角 " << rad2str(MSC::mPJ, false) << "  " << rad2str(MSC::sPJ, false) << "\n";
    ss << "高度角 " << rad2str(MSC::mPW, false) << "  " << rad2str(MSC::sPW, false) << "\n";
    ss << "时角   " << rad2str(MSC::mShiJ, false) << "  " << rad2str(MSC::sShiJ, false) << "\n";
    ss << "视半径   " << m2fm(MSC::mRad, 2, 0) << "       " << m2fm(MSC::sRad, 2, 0) << " (观测点)\n";

    if (fs) {
        ss << "-------------------------------------------\n";
        ss << "力学时" << JD2str(MSC::T + J2000);
        ss << " ΔT=" << to_str(MSC::dt * 86400, 1) << "秒\n";
        ss << "黄经章 " << to_str(MSC::dL / pi2 * 360 * 3600, 2) << "\" ";
        ss << "交角章 " << to_str(MSC::dE / pi2 * 360 * 3600, 2) << "\" ";
        ss << "ε=" << rad2str(MSC::E, false) << "\n";
        ss << "-------------------------------------------\n";
    }
    return ss.str();
}