...
{  //创建插值表(根数表)
    if (suoN(jd) == suoN(RS_GS::Zjd) && RS_GS::Zs.size() == n * 9)
        return;
    RS_GS::Zs.clear();
    ...
    std::array<double, 3> S, M;
    std::vector<double> a(n * 9);

    for (int i = 0; i < n; i++) {  //插值点范围不要超过360度(约1个月)
        double T = (RS_GS::Zjd + (i - n / 2.0 + 0.5) * RS_GS::Zdt) / 36525.0;
        ...
        std::array<double, 3> B = xyz2llr({S[0] - M[0], S[1] - M[1], S[2] - M[2]});
        B[0] = _pi / 2 + B[0];
        B[1] = _pi / 2 - B[1];
        if (i && B[0] < a[6])
            B[0] += pi2;  //确保插值数据连续
...
std::array<double, 3> RS_GS::cd2bse(const std::array<double, 3> &z, const std::array<double, 3> &I)
{  //赤道转贝塞尔坐标
    std::array<double, 3> r = {z[0] - I[0], z[1], z[2]};
    r = llrConv(r, -I[1]);
    return llr2xyz(r);
}
std::array<double, 3> RS_GS::bse2cd(const std::array<double, 3> &z, const std::array<double, 3> &I)
{  //贝塞尔转赤道坐标
...
std::array<double, 3> RS_GS::bse2db(const std::array<double, 3> &z, const std::array<double, 3> &I, bool f)
...
std::array<double, 3> RS_GS::bseXY2db(double x, double y, const std::array<double, 3> &I, bool f)
...
{  //月亮的贝塞尔坐标
    std::array<double, 3> a = RS_GS::cd2bse(RS_GS::chazhi(jd, 1), RS_GS::chazhi(jd, 2));
...
{  //求切入点
    ...
    x += c * y;
    y -= c * x;
    std::array<double, 3> re = bse2db({x / d, y / d, 0}, bse(jd), true);
...
{                     //日食的基本特征
    ...
    double ls;
    std::array<double, 3> a = bseM(jd - tg);
    std::array<double, 3> b = bseM(jd);
    std::array<double, 3> c = bseM(jd + tg);
    ...
    if (re.d < 1) {
        re.gk1 = qrd(t2 + jd, vx, vy, false);  //中心始
        re.gk2 = qrd(t3 + jd, vx, vy, false);  //中心终
    } else {
        re.gk1 = {0, 0, 0};
        re.gk2 = {0, 0, 0};
    }

    re.gk3 = qrd(t4 + jd, vx, vy, true);  //偏食始
    re.gk4 = qrd(t5 + jd, vx, vy, true);  //偏食终
    re.gk5 = bseXY2db(t6 * vx + b[0], t6 * vy + b[1], bse(t6 + jd), true);
    ...
    if (F.W == 100) {
        ...
        //无中心线
        //最大食地标及时分
        lls = bse2db({re.xc, re.yc, 0}, re.I, false);
...
//界线图
void RS_GS::push(const std::array<double, 3> &z, std::vector<double> &p)
{
...
std::array<double, 4> RS_GS::nanbei(const std::array<double, 3> &M, double vx0, double vy0, double h, double r,
                                    const std::array<double, 3> &I)
{  //vx0,vy0为影足速度(也是整个影子速度),h=1计算北界,h=-1计算南界
    double x = M[0] - vy0 / vx0 * r * h, y = M[1] + h * r;
    ...
    for (int i = 0, js = 0; i < 3; i++) {
        ...
        x -= (x - M[0]) * z / M[2];
        y -= (y - M[1]) * z / M[2];
        double vx = vx0 - pi2 * (sin(I[1]) * z - cos(I[1]) * y);
        double vy = vy0 - pi2 * cos(I[1]) * x;
...
bool RS_GS::mDian(const std::array<double, 3> &M, double vx0, double vy0, bool AB, double r, const std::array<double, 3> &I,
                  std::vector<double> &A)
{  //日出日没食甚
    double R = 0;
    NODE p;
    ...
    for (int i = 0; i < 2; i++) {  //迭代求交点
        ...
        if (!p.n)
            break;
        if (AB)
            a = {p.A[0], p.A[1]}, R = p.R1;
        else
            a = {p.B[0], p.B[1]}, R = p.R2;
    }
    if (p.n && R <= r) {                       //有交点
        a = bse2db({a[0], a[1], 0}, I, true);  //转为地标
...
{  //界线表
    ...
    for (int i = 0; i < N; i++, t += dt) {
        ...
        std::array<double, 3> M = bseM(t);  //此刻月亮贝塞尔坐标(其x和y正是影足)
        _RSM B = rSM(M[2]);                 //本半影等`
        double r = B.r1;                    //半影半径
...
{ //vx0,vy0为影足速度(也是整个影子速度),h=1计算北界,h=-1计算南界
   ...
   { //补线头线尾
     ...
     if(g.n){
      ...
      RS_GS::push( bse2db(F,I2,1), A);//有解补线头
     }
   }
...
{ //日出日没的初亏食甚复圆线，南北界线等
  ...
  for(i=0;i<=N;i++,t+=dt)
  {
   ...
   if(p.n) { //有交点
    p.A[2]=p.B[2]=0;  p.A=bse2db(p.A,I,1);  p.B=bse2db(p.B,I,1); //转为地标
   ...
   //求中心线
   std::array<double, 3> lls;
   std::array<double, 3> pp = bseXY2db(M[0],M[1],I,1);
   ...
   { //从无交点跳到有交点或反之
     ...
     RS_GS::push( bse2db(lls,I2,1), re.L0 );
     n4++;
   }
...
{ //jd力学时
  ...
  std::array<double, 3> M = bseM(jd);  //此刻月亮
  _RSM B = rSM(M[2]); //本半影等
  ...
  {//第0和第N点是同一点，可形成一个环，但不必计算，因为第0点可能在界外而无效
    ...
    else { if(sqrt(x*x+y*y)>a1) RS_GS::push( bse2db({x,y,0},I,1), p1 ); }
    //半影
    x = M[0] + B.r1*cosS, y = M[1] + B.r1*sinS;
    p = lineEar2(X,Y,Z,  x,y,0,  cs_ba,1,I);
    if(p.W!=100) RS_GS::push( {p.J,p.W}, p2 );
    else { if(sqrt(x*x+y*y)>a2) RS_GS::push( bse2db({x,y,0},I,1), p2 ); }
    //晨昏圈
