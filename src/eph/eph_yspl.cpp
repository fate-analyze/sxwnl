...
double YS_PL::lineT(const RE0 &G, double v, double u, double r, bool n)
...
{  //月食的食甚计算(jd为近朔的力学时,误差几天不要紧)
    //YS_PL::lT={};
    for (int i = 0; i < 7; i++)
        YS_PL::lT[i] = 0;  //分别是:食甚,初亏,复圆,半影食始,半影食终,食既,生光
    YS_PL::sf = 0;
    YS_PL::LX = "";
    ...
    if (rmin <= G.mr + G.er) {  //食计算
        ...
        YS_PL::lT[0] = lineT(G, v, u, G.mr + G.er, false);  //初亏
        YS_PL::lecXY(YS_PL::lT[0], g);
        YS_PL::lT[0] = lineT(g, v, u, g.mr + g.er, false);  //初亏再算一次
                                                            //	std::cout<<YS_PL::lT[0]<<std::endl;
        YS_PL::lT[2] = lineT(G, v, u, G.mr + G.er, true);   //复圆
        YS_PL::lecXY(YS_PL::lT[2], g);
        YS_PL::lT[2] = lineT(g, v, u, g.mr + g.er, true);  //复圆再算一次
    }
    if (rmin <= G.mr + G.Er) {                              //半影食计算
        YS_PL::lT[3] = lineT(G, v, u, G.mr + G.Er, false);  //半影食始
        YS_PL::lecXY(YS_PL::lT[3], g);
        YS_PL::lT[3] = lineT(g, v, u, g.mr + g.Er, false);  //半影食始再算一次

        YS_PL::lT[4] = lineT(G, v, u, G.mr + G.Er, true);  //半影食终
        YS_PL::lecXY(YS_PL::lT[4], g);
        YS_PL::lT[4] = lineT(g, v, u, g.mr + g.Er, true);  //半影食终再算一次
    }
    if (rmin <= G.er - G.mr) {  //全食计算
        YS_PL::LX = "全";
        YS_PL::lT[5] = lineT(G, v, u, G.er - G.mr, false);  //食既
        YS_PL::lecXY(YS_PL::lT[5], g);
        YS_PL::lT[5] = lineT(g, v, u, g.er - g.mr, false);  //食既再算一次

        YS_PL::lT[6] = lineT(G, v, u, G.er - G.mr, true);  //生光
        YS_PL::lecXY(YS_PL::lT[6], g);
        YS_PL::lT[6] = lineT(g, v, u, g.er - g.mr, true);  //生光再算一次
    }
}