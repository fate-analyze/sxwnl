...
std::vector<std::string> OBA::wFtv = {
    ...
    "0520.国际母亲节",   "0530I全国助残日", "0630.父亲节",
    "0730.被奴役国家周", "0932I国际和平日", "0940.国际聋人节 世界儿童日",
    "0950I世界海事日",   "1011.国际住房日", "1013I国际减轻自然灾害日(减灾日)",
    "1144I感恩节"};
...
void OBA::getDayName(OB_DAY &r)
{
    ...
    *****************/
    std::string m0 = (r.m < 10 ? "0" : "") + std::to_string(r.m);
    std::string d0 = (r.d < 10 ? "0" : "") + std::to_string(r.d);
    std::string s, type;

    if (r.week == 0 || r.week == 6) {
        r.Fjia = true;  //星期日或星期六放假
    }

    //按公历日期查找
    for (int i = 0; i < sFtv[r.m - 1].size(); i++) {
        //公历节日或纪念日,遍历本月节日表
        s = sFtv[r.m - 1][i];
        if (s.substr(0, 2) != d0) {
            continue;
        ...
        if (s.substr(5, 1) == "-") {
            //有年限的
            if (r.y < std::stoi(s.substr(1, 4)) || r.y > std::stoi(s.substr(6, 4))) {
                continue;
            }
    ...
    if (r.week >= r.week0)
        _w += 1;
    int _w2 = _w;
    if (r.weeki == r.weekN - 1)
        _w2 = 5;
    std::string w = m0 + std::to_string(_w) + std::to_string(r.week);  //d日在本月的第几个星期某
    std::string w2 = m0 + std::to_string(_w2) + std::to_string(r.week);

    for (int i = 0; i < wFtv.size(); i++) {
        s = wFtv[i];
...
std::string OBB::getNH(int y)
{
    ...
    for (int i = 0; i < ob.size(); i += 7) {
        int j = std::stoi(ob[i]);
        if (y < j || y >= j + std::stoi(ob[i + 1]))
            continue;
        std::string c = ob[i + 6] + std::to_string(y - j + 1 + std::stoi(ob[i + 2])) + "年";  //年号及年次
...
void OBB::getDayName2(OB_DAY &r)
{
    ...
    if (r.Lleap != "闰") {
        if (d == "正月初一")
            r.A += "春节 ", r.Fjia = true;  //Fjia(放假)
        if (d == "正月初二")
            r.B += "大年初二 ", r.Fjia = true;
        if (d == "五月初五")
            r.A += "端午节 ", r.Fjia = true;
        if (d == "八月十五")
            r.A += "中秋节 ", r.Fjia = true;
    ...
    if (r.cur_dz >= 0 && r.cur_dz < 81) {
        ...
        if (r.cur_dz % 9 == 0)
            ...
            r.C += w + "九第" + std::to_string(r.cur_dz % 9 + 1) + "天 ";
    }

    w = r.Lday2.substr(0, 3);
    std::string w2 = r.Lday2.substr(3, 6);
    if (r.cur_xz >= 20 && r.cur_xz < 30 && w == "庚")
        r.B += "初伏 ";
    if (r.cur_xz >= 30 && r.cur_xz < 40 && w == "庚")
        r.B += "中伏 ";
    if (r.cur_lq >= 0 && r.cur_lq < 10 && w == "庚")
        r.B += "末伏 ";
    if (r.cur_mz >= 0 && r.cur_mz < 10 && w == "丙")
        r.B += "入梅 ";
    if (r.cur_xs >= 0 && r.cur_xs < 12 && w2 == "未")
        r.B += "出梅 ";
}
