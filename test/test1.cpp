...
void tianXiang(int xm, int xm2, Date dat, int n = 10)
{
    ...
    double jd = date2Jd({dat.year_, dat.month_, dat.day_}) - J2000;  //取屏幕时间
    std::string s = "";
    int i;
    double re0;
...
void dingQi_cmp(int y = 2000, int N = 10)
{
    ...
    std::cout << "\033[31;1m" << std::to_string(2000 + y) << "年之后" << std::to_string(N)
              << "个朔日粗算与精算的最大差异:" << std::to_string(maxT) << "秒。\033[0m" << std::endl;
}
...
Date get_time(void)
{
    ...
    time0 = time(NULL);
    bjs = localtime(&time0);
    Date dat = {bjs->tm_year + 1900, bjs->tm_mon + 1, bjs->tm_mday, bjs->tm_hour, bjs->tm_min, (double)bjs->tm_sec};
    return dat;
}

void initmap(OB_LUN lun)
{
    std::unordered_map<std::string, std::string> str_yx = {
        {"望",   "\033[33m●"      },
        {"上弦", "\033[33m∪"      },
        {"朔",   "\033[38;5;245m●"},
        {"下弦", "\033[33m∩"      }
    ...
    for (int i = 0; i < lun.dayNum_; i++) {
        ...
        if (!j)
            strmap[y * 3][x] += "\033[0m\033[1m";
        if (lun.day[i].Ldi == 0)
            strmap[y * 3 + 1][x] = "\033[38;5;123m",
                           strmap[y * 3 + 1][x] = strmap[y * 3 + 1][x] + lun.day[i].Lleap + strmap[y * 3 + 1][x] + lun.day[i].Lmc
                                                  + strmap[y * 3 + 1][x] + (lun.day[i].Lmc.length() == 6 ? "" : "月") + strmap[y * 3 + 1][x]
                                                  + str_dx[lun.day[i].Ldn - 29] + strmap[y * 3 + 1][x]
                                                  + (lun.day[i].Lleap.length() ? "\033[0m\033[1m" : "  \033[0m\033[1m");
        else
            strmap[y * 3 + 1][x] = "",
                           strmap[y * 3 + 1][x] = strmap[y * 3 + 1][x] + lun.day[i].Ldc + strmap[y * 3 + 1][x] + "    \033[0m\033[1m";
        if (lun.day[i].Ljq.length())
            strmap[y * 3 + 1][x] = "\033[34m",
                           strmap[y * 3 + 1][x] = strmap[y * 3 + 1][x] + lun.day[i].Ljq + strmap[y * 3 + 1][x] += "    \033[0m\033[1m";
    }
...
int main()
{
    ...
    */
    ...
    // 升降
    std::cout << "\n========--升降计算--========\n";
    std::cout << shengjiang(dat.year_, dat.month_, dat.day_) << "\n";
